var Endless = function() {
	...
	this.draw = function(ctx) {
				...
				if (_state == INPLAY) {
					...
					if (j == 0 && model.get(i,j).type < 0) {
						...
						if (levelchanged) {
							...
							levelchanged = false;
							if (!mute) resourceRepository.schwing.play();
						}
						var tile = new Tile();
		...
		if (_state == WAITING) {
			...
			if (_frame < 50) {
				...
				var txt = "a dead pixel.";
				writeMessage(ctx, txt, WHITE, (_width-fs*txt.length)/2, starth + (fs*(4)), fs);

				var txt = "dead pixel";
				writeMessage(ctx, txt, LOGO, (_width-fs*txt.length)/2, starth + (fs*(6)), fs);

				var index = 20;//gray
				ctx.drawImage(resourceRepository.tileSheet, 64*(index%8)+1, 64*Math.floor(index/8)+1, 63, 63, w/2 - fs/2, starth + (fs*(29/4)), fs*3, fs*3);

				var txt = "dead pixels can";
				writeMessage(ctx, txt, WHITE, (_width-fs*txt.length)/2, starth + (fs*(11)), fs);
				var txt = "not be rotated but";
				writeMessage(ctx, txt, WHITE, (_width-fs*txt.length)/2, starth + (fs*(12)), fs);
				var txt = "can still score";
				writeMessage(ctx, txt, WHITE, (_width-fs*txt.length)/2, starth + (fs*(13)), fs);
				var txt = "when lined up.";
				writeMessage(ctx, txt, WHITE, (_width-fs*txt.length)/2, starth + (fs*(14)), fs);
			}
		}
	...
	this.update = function() {
		...
		if (_state == INPLAY && clicks >= TOOMANYCLICKS) {
			_state = GAMEOVER;
			_frame = 0;
	...
	save = function() {
		if (typeof(Storage) !== "undefined") {
			...
			localStorage.setItem("endlesshighlevel", highlevel);
		}
		else {
	...
	load = function() {
		highscore = Number(localStorage.getItem("endlesshighscore"));
		highlevel = Number(localStorage.getItem("endlesshighlevel"));
	};
	...
	setVolume = function(v) {
		...
		resourceRepository.schwing.volume = volume;
	};
