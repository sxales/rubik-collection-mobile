var Arcade = function() {
	...
	this.init = function(w,h) {
		...
		_w = 6;
		_h = Math.floor((_height*SCREENRATIO) / (_width / 6));
	...
	this.draw = function(ctx) {
		...
		if (_state == WAITING) {
			...
			writeMessage(ctx, txt, PINK, (_width-fs*txt.length)/2, starth + (fs*(3)), fs);
			...
			var txt = "highscore";
			writeMessage(ctx, txt, LOGO, margin+fs, starth + (fs*(7)), fs);
			var txt = "level";
			writeMessage(ctx, txt, LOGO, (_width-fs*txt.length)-margin-fs, starth + (fs*(7)), fs);

			var txt = zeroFill(highscore, 5);
			writeMessage(ctx, txt, WHITE, margin+(fs*3), starth + (fs*(17/2)), fs);
			var txt = zeroFill(level+1, 5);
			writeMessage(ctx, txt, WHITE, (_width-fs*txt.length)-margin-fs, starth + (fs*(17/2)), fs);
		...
		writeMessage(ctx, "time", RED, margin+(fs/2), position+(fs*2), fs);
		var txt = zeroFill(time, 3);
		writeMessage(ctx, txt, WHITE, margin+(fs), position+(fs*3), fs);

		writeMessage(ctx, "lines", RED, margin+(fs*8), position, fs);
		var lines = linestoclear - levellines;
		if (lines < 0) lines = 0;
		var txt = zeroFill(lines, 3);
		writeMessage(ctx, txt, WHITE, margin+(fs*8)+(fs), position+fs, fs);

		writeMessage(ctx, "level", RED, margin+(fs*8), position+(fs*2), fs);
		var txt = zeroFill(level+1, 3);
		writeMessage(ctx, txt, WHITE, margin+(fs*8)+fs, position+(fs*3), fs);
	...
	save = function() {
		if (typeof(Storage) !== "undefined") {
			...
			localStorage.setItem("arcadehighlevel", highlevel);
		}
		else {
	...
	load = function() {
		highscore = Number(localStorage.getItem("arcadehighscore"));
		highlevel = Number(localStorage.getItem("arcadehighlevel"));
	};
	...
	calculatePoints = function(count) {
		...
		levellines++;
		if (levellines >= linestoclear) levelchanged = true;
